  ...
  <PageContext>
    <main id="all">
      <section class="flex-center">
          ...
          <template v-slot:other-items v-if="isDev">
            ...
            >
              <Icon
                ...
                class="filter-icon icon"
                role="button"
                :data-active="filterMenuActive"
              />
        ...
        <p
          v-else-if="active_tab.toLowerCase() == 'all'"
          style="text-align: center"
        >
          The entire list of my projects* <br />
          <i
            >(more can be found on my
            <a href="https://github.com/ultraflame4?tab=repositories">github</a
            >)</i
          >
        </p>
        <p
          v-else-if="active_tab.toLowerCase() == 'completed'"
          style="text-align: center"
        >
          Projects that I consider to be completed & released.
        </p>
        <p
          v-else-if="active_tab.toLowerCase() == 'in dev'"
          style="text-align: center"
        >
          Projects that in active development or likely to have major changes in
          the future.
        </p>
        <p
          v-else-if="active_tab.toLowerCase() == 'inactive'"
          style="text-align: center"
        >
          Incomplete projects that have been put on hold due to various reasons
          including but not limited to<br />
          1. lack of time, 2. loss of interest & motivation, 3. technical
          difficulties
        </p>
...
const project_list = computed<FuseResult<NormalisedProjectData>[]>(() => {
  ...
  let results =
    searchTerm.value.trim().length > 0
      ? fuse.search(searchTerm.value)
      : projects_as_results.value;
